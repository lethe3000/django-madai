...
class ClientAppDatatablesBuilder(DatatablesBuilder):
    ...
    os = DatatablesTextColumn(label=u'操作系统',
                              is_searchable=True,
                              col_width="7%")
    ...
    app_version_code = DatatablesTextColumn(label=u'版本号',
                                            is_sortable=True)

    app_version_name = DatatablesTextColumn(label=u'版本名称')

    is_force_upgrade = DatatablesBooleanColumn(label=u'强制更新')

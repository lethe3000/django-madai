...
"""
    def save(self, commit=False):
        ...
        if hotel.content_file:
            # update it if has content file
            with open(hotel.content_file.path, 'w') as f:
                f.write(self.cleaned_data['content_html'].encode('utf-8'))
        else:
            buf = StringIO.StringIO(self.cleaned_data['content_html'].encode('utf-8'))
            hotel.content_file = SimpleUploadedFile("content.html", self.RESPONSIVE_HEADER + buf.read())

        if not hasattr(hotel, "creator"):
            hotel.creator = self.initial['request'].user
        hotel.save()
...
class HotelDatatablesBuilder(DatatablesBuilder):
    ...
    name = DatatablesTextColumn(label=u'酒店名',
                                is_searchable=True)

    price = DatatablesTextColumn(label=u'价格',
                                 is_searchable=True)

    address = DatatablesTextColumn(label=u'联系地址',
                                   is_searchable=True)

    # phone = DatatablesTextColumn(label=u'电话号码',
    #                              is_searchable=True)

...
class HotelListView(TemplateResponseMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        query_set = Hotel.active_objects.filter(is_published=True, is_special=False) if not filter or filter == 'all'\
            else Hotel.active_objects.filter(short_index=filter, is_published=True, is_special=False)
...
class HotelPromotionListView(HotelListView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        page_size = 6
        filter = request.GET.get('filter')
        query_set = Hotel.active_objects.filter(is_published=True, is_promotion=True, is_special=False) if not filter or filter == 'all' else Hotel.active_objects.filter(short_index=filter, is_published=True, is_promotion=True, is_special=False)
